  ...
  try {
    ...
    if (itemsToAnalyze.length === 0) {
      return NextResponse.json({
        message: "Risk scores up to date",
        updated: 0,
      });
    }
    ...
    if (complexItems.length > 0) {
      ...
      const prompt = `
        ...
        Context:
        - High Heat/Humidity (${seasonContext}) drastically increases risk for Dairy, Berries, Wheaty Greens, Fish.
