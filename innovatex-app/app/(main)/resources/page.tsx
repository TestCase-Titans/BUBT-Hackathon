    ...
    if (resources.length > 0) {
      ...
      resources.forEach((res) => {
        const isRelevant =
          ...
          Array.from(userCategories).some((userCat) =>
            userCat.includes(res.category)
          );
  ...
  return (
    <PageWrapper>
      <div className="pb-24 lg:pb-8 pt-8 px-6 lg:px-12 overflow-y-auto h-full max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="bg-[#0A3323] p-6 rounded-2xl text-[#F3F6F4] text-center flex flex-col justify-center shadow-lg transform transition-transform hover:scale-[1.02]">
                ...
                <button
                  ...
                  className="w-full py-3 bg-[#D4FF47] text-[#0A3323] rounded-xl font-bold text-sm hover:bg-white transition-colors"
                >
                  Join Now
                </button>
              </div>
        ...
        {others.length > 0 && (
          <div>
            <h3 className="text-xl font-bold text-[#0A3323] mb-6 border-t border-gray-200 pt-8">
              Explore More
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-6">
                  <span className="px-3 py-1 bg-[#D4FF47] text-[#0A3323] text-xs font-bold rounded-full uppercase tracking-wide">
                    {selectedResource.category}
                  </span>
                  {selectedResource.isAiGenerated && (
                    <span className="px-2 py-1 bg-blue-50 text-blue-600 text-[10px] font-bold rounded-full flex items-center gap-1">
                      <Sparkles size={10} /> AI Generated
                    </span>
                  )}
                </div>

                <h2 className="text-3xl font-serif text-[#0A3323] mb-6 leading-tight">
                  ...
                  {selectedResource.content ? (
                    <ReactMarkdown
                      components={{
                        h1: ({ node, ...props }) => (
                          <h1
                            className="text-2xl font-bold text-[#0A3323] mt-6 mb-4"
                            {...props}
                          />
                        ),
                        h2: ({ node, ...props }) => (
                          <h2
                            className="text-xl font-bold text-[#0A3323] mt-5 mb-3"
                            {...props}
                          />
                        ),
                        ul: ({ node, ...props }) => (
                        ...
                        strong: ({ node, ...props }) => (
                          <span
                            className="font-bold text-[#0A3323]"
                            {...props}
                          />
                        ),
                      }}
                ...
                <div className="mt-8 flex flex-col sm:flex-row gap-3 border-t border-gray-100 pt-6">
                  {(!selectedResource.content ||
                    selectedResource.content.length < 200) &&
...
function ResourceCard({
  ...
  const isExternal =
    !!validUrl && (!resource.content || resource.content.length < 50);
  ...
  return (
    <motion.div
      ...
      onClick={handleClick}
      className={`bg-white p-6 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border group cursor-pointer flex flex-col h-full relative overflow-hidden ${
        recommended
          ? "border-[#D4FF47] ring-1 ring-[#D4FF47]/20"
          : "border-gray-100"
      }`}
    >
      ...
      <h3 className="font-bold text-xl text-[#0A3323] mb-3 leading-tight relative z-10 line-clamp-2">
        {resource.title}
      </h3>

      <p className="text-sm text-gray-500 leading-relaxed mb-4 flex-1 line-clamp-3 relative z-10">
        {resource.description || resource.content || "Tap to view details."}
      </p>

      <div className="flex items-center text-[#0A3323] text-xs font-bold mt-auto relative z-10">
        <span className="group-hover:underline">
          {isExternal ? "Open Link" : "Read Full Guide"}
        </span>
        {isExternal ? (
          <ExternalLink
            size={14}
